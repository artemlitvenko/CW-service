        ...
        <div className="popup popup-edit" onClick={() => dispatch(setPopupEditDisplayMaster(false))}>
            <div className="popup-content" onClick={(event => event.stopPropagation())}>
                <div className="popup-header">
                    ...
                    <button className="popup-close" onClick={() => dispatch(setPopupEditDisplayMaster(false))}>X
                    </button>
                </div>
                <Input type="text" name="name" placeholder="Master name" value={editMasterName.name}
                       setValue={editSetMasterName}/>
                <h4>Change rating of master</h4>
                <select name="rating" value={editMasterRating.rating}
                        onChange={(event) => editSetMasterRating(event.target.value)}>
